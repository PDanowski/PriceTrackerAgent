...
export const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onAddProduct }) => {
  ...
  const handleScrape = async () => {
    if (!url || !url.startsWith('http')) {
      setScrapeError('Please enter a valid product URL (HTTP/HTTPS)');
      return;
    }
    ...
    try {
      ...
      setScrapeError(err.message || 'Failed to fetch data from product link');
    } finally {
      setIsScraping(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full border border-slate-200 shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">Add Product Link</h3>
              <p className="text-xs text-slate-500">Paste URL from any online store</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
          {/* Step 1: URL input */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Product URL</label>
            <div className="flex gap-2">
              <input
                type="url"
                required
                placeholder="https://www.store.com/product/123"
                value={url}
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100">
            {hasScraped && !scrapeNotice && (
              <div className="bg-emerald-50 border border-emerald-200/80 rounded-xl p-3 flex items-center space-x-2 text-xs text-emerald-800">
                ...
                <span>Fetched product details! You can verify the values below.</span>
              </div>
            )}
            ...
            {scrapeNotice && (
              <div className="bg-amber-50 border border-amber-200/90 rounded-xl p-3 text-xs text-amber-900 leading-relaxed">
                <div className="font-semibold text-amber-900 mb-0.5 flex items-center space-x-1">
                  <span>Store Protection Notice</span>
                </div>
            ...
            {imageUrl && (
              <div className="flex items-center space-x-3 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                ...
                <div className="text-xs text-slate-600 overflow-hidden">
                  <span className="font-semibold text-slate-800 block">Fetched Product Image</span>
